...
st.title('Estratégia Kubernetes x Uso de Recursos')
...
with col5_1:
    ...
    df1 = cria_dataframe(cpu_trigger, min_replicas, max_replicas)
    bola1 = int(round((cpu_usage / cpu_limit) * 100, 0))
    z1 = int(df1[(df1[0] == bola1)][2])

    fig1 = go.Figure(data=[go.Scatter3d(z=df1[2].values, x=df1[0].values, y=df1[1].values, mode='lines', name='estratégia CPU')])
...
with col5_2:
    ...
    df2 = cria_dataframe(memory_trigger, min_replicas, max_replicas)
    bola2 = int(round((memory_usage / memory_limit) * 100, 0))
    z2 = int(df2[(df2[1] == bola2)][2])

    fig2 = go.Figure(data=[go.Scatter3d(z=df2[2].values, x=df2[0].values, y=df2[1].values, mode='lines', name='estratégia Memória')])
